...
class Api extends CI_Controller {
	...
	private function getPrice($user_id, $shop_id, $product_id){
		$QProduct = $this->db
				->where("id",$product_id)
				->get("tb_product")
				->row();
	...
	private function getShopById($id,$user=null){
		...
		*/
		$QShopProducts = $this->db
						...
						->where(" ((tp.stock_type = 0 AND tp.end_date >= '".date("Y-m-d")."') OR tp.stock_type = 1) ",null,false)
			...
			*/
			...
			foreach($QCartProducts as $QCartProduct){
				...
				*/
				...
				/*
				*	------------------------------------------------------------------------------
				*	Menyimpan hasil kalkulasi cart product price
				*	------------------------------------------------------------------------------
				*/
				$Data = array("price_product"=>$priceCartProduct);
				$Save = $this->db->where("id",$QCartProduct->id)->update("tb_cart_product",$Data);
				$priceCart = $priceCart + $priceCartProduct;
			...
			/*
			*	------------------------------------------------------------------------------
			*	Menyimpan hasil kalkulasi cart price
			*	------------------------------------------------------------------------------
			*/
			$Data = array("price_total"=>$priceCart);
	...
	*/
			...
			*/
			...
			if($this->response->post("stock_type") != "" && $this->response->postDecode("stock_type") != ""){
				if($this->response->postDecode("stock_type") == 0){
					...
					$QProduct = $QProduct->where("tp.end_date >= ",date("Y-m-d"));
				}else{
					$QProduct = $QProduct->where("tp.stock_type",$this->response->postDecode("stock_type"));
				}
			}else{
	...
	private function isCardAddValid(){
		...
		*/
		...
		if($this->response->post("product") == "" || $this->response->postDecode("product") == ""){
			$this->response->send(array("result"=>0,"message"=>"Belum ada product yang di kirim [3]","messageCode"=>3), true);
			return false;
		}
		...
		if(empty($QProduct)){
			$this->response->send(array("result"=>0,"message"=>"Barang sudah tidak tersedia [4]","messageCode"=>4), true);
			return false;
		}
		
		if($this->response->post("varians") == "" || $this->response->postDecode("varians") == ""){
			$this->response->send(array("result"=>0,"message"=>"Tidak ada data varians yang dikirim [5]","messageCode"=>5), true);
			return false;
		}
		
		if($this->response->post("stock_type") == "" || $this->response->postDecode("stock_type") == ""){
			$this->response->send(array("result"=>0,"message"=>"Tidak ada data tipe stok [6]","messageCode"=>6), true);
			return false;
		}
		
		if($this->response->post("price") == "" || $this->response->postDecode("price") == ""){
			$this->response->send(array("result"=>0,"message"=>"Tidak ada data harga barang [7]","messageCode"=>7), true);
			return false;
		}
		
		if($this->response->post("min_order") == "" || $this->response->postDecode("min_order") == ""){
			$this->response->send(array("result"=>0,"message"=>"Tidak ada data minimal order [8]","messageCode"=>8), true);
			return false;
		}
		
		if($QProduct->active == 0){
			$this->response->send(array("result"=>0,"message"=>"Barang tidak tersedia [9]","messageCode"=>9), true);
			return false;
		}
		...
		*/
		...
		if(empty($QShopMember)){
			$this->response->send(array("result"=>0,"message"=>"Anda sudah tidak tergabung dengan toko \"".$QProduct->shop_name."\"  [12]","messageCode"=>12), true);
			return false;
		}
		...
		if(empty($QShop)){
			$this->response->send(array("result"=>0,"message"=>"Produk dari toko \"".$QProduct->shop_name."\" sudah tidak valid  [13]","messageCode"=>13), true);
			return false;
		}
		...
		if($product_price != (float) $this->response->postDecode("price")){
			$this->response->send(array("result"=>0,"message"=>"Harga sudah berubah [14]","messageCode"=>14), true);
			return false;
		}
		
		if($QProduct->min_order != (float) $this->response->postDecode("min_order")){
			$this->response->send(array("result"=>0,"message"=>"Data produk sudah berubah [15]","messageCode"=>15), true);
			return false;
		}
		...
		*/
		...
		for($i=1;$i<=$this->response->postDecode("varians");$i++){
			if($this->response->post("varian".$i) != "" && $this->response->postDecode("varian".$i) != "" && $this->response->post("varian".$i."_qty") != "" && $this->response->postDecode("varian".$i."_qty") != "" && $this->response->postDecode("varian".$i."_qty") != "0"){
				...
				if(empty($QVarian)){
					$this->response->send(array("result"=>0,"message"=>"Barang tidak tersedia [16]","messageCode"=>16), true);
					$isVarianValid = false;
					continue;
				}else{
					/*
					...
					*/
					if($QProduct->stock_type == 1 && $QProduct->stock_type_detail == 0){
						if($QVarian->stock_qty < (float) $this->response->postDecode("varian".$i."_qty")){
							$this->response->send(array("result"=>0,"message"=>"Stok barang tidak tersedia [17]","messageCode"=>17), true);
							$isVarianValid = false;
							continue;
						}
					}
		...
		if($QProduct->min_order > $buy_qty){
			$this->response->send(array("result"=>0,"message"=>"Jumlah pembelian kurang dari minimal order [18]","messageCode"=>18), true);
			return false;
		}
	...
	public function doCartAdd(){
		try {
			...
			*/
			...
			/*
			*	------------------------------------------------------------------------------
			*	Mengambil data cart berdasarkan user, toko dan stock type
			*	------------------------------------------------------------------------------
			*/
			$QCart = $this->db
						->where("member_id",$QUser->id)
				...
				foreach($QCartProducts as $QCartProduct){
					...
					*/
					...
					foreach($QCartVarians as $QCartVarian){
						/*
						*	------------------------------------------------------------------------------
						*	Mengambil data product varian berdasarkan product
						*	------------------------------------------------------------------------------
						*/
						$QProductVarian = $this->db
											->where("id",$QCartVarian->product_varian_id)
						...
						*/
						...
						/*
						*	------------------------------------------------------------------------------
						*	Menghitung harga pembelian produk berdasarkan cart varian quantity dan harga product
						*	------------------------------------------------------------------------------
						*/
						$UserPrice = $this->getPrice($QUser->id,$QProduct->shop_id,$QProduct->id);
						$countPriceProduct = $countPriceProduct + ($QCartVarian->quantity * $UserPrice);
					}
					
					/*
					*	------------------------------------------------------------------------------
					*	Menyimpan harga pembelian per produk
					*	------------------------------------------------------------------------------
					*/
					
					$Data = array("price_product"=>$countPriceProduct);
					$Save = $this->db->where("id",$QCartProduct->id)->update("tb_cart_product",$Data);
					...
					*/
					$CartProduct = array(
									"id"=>$QCartProduct->id,
									"price_product"=>$countPriceProduct,
					...
					/*
					*	------------------------------------------------------------------------------
					*	Menghitung harga pembelian cart berdasarkan total harga pembelian per product
					*	------------------------------------------------------------------------------
					*/
					
					$countPriceCart = $countPriceCart + $countPriceProduct;
				}
				
				/*
				*	------------------------------------------------------------------------------
				*	Menyimpan harga pembelian per cart
				*	------------------------------------------------------------------------------
				*/
				
				$Data = array("price_total"=>$countPriceCart);
				$Save = $this->db->where("id",$QCart->id)->update("tb_cart",$Data);
		...
		try{
			...
			*/
			...
			for($p=0;$p<= ((float) $this->response->postDecode("cart_products")) - 1;$p++){
				if($this->response->post("cart_product".$p) != "" || $this->response->postDecode("cart_product".$p) != ""){
					...
					if($QProduct->stock_type == 0){
						if(empty($MessageProduct)){
							$isCartValid = false;
							$MessageProduct = array("product"=>$FormCartProduct,"message"=>"Data barang telah berubah [13]");
							array_push($MessageProducts,$MessageProduct);
						}
					}
					...
					if($this->response->post("cart_product".$p."_stock_type") == "" || $this->response->postDecode("cart_product".$p."_stock_type") == ""){
						if(empty($MessageProduct)){
							$isCartValid = false;
							$MessageProduct = array("product"=>$FormCartProduct,"message"=>"Tidak ada data stock type yang dikirim [14]");
							array_push($MessageProducts,$MessageProduct);
						}
					}
					...
					if($this->response->post("cart_product".$p."_min_order") == "" || $this->response->postDecode("cart_product".$p."_min_order") == ""){
						if(empty($MessageProduct)){
							$isCartValid = false;
							$MessageProduct = array("product"=>$FormCartProduct,"message"=>"Tidak ada data minimal order yang dikirim [15]");
							array_push($MessageProducts,$MessageProduct);
						}
					}
					...
					if($this->response->post("cart_product".$p."_price") == "" || $this->response->postDecode("cart_product".$p."_price") == ""){
						if(empty($MessageProduct)){
							$isCartValid = false;
							$MessageProduct = array("product"=>$FormCartProduct,"message"=>"Tidak ada data harga yang dikirim [16]");
							array_push($MessageProducts,$MessageProduct);
						}
					}
					...
					*/
					...
					if($this->response->post("cart_product".$p."_varians") == "" || $this->response->postDecode("cart_product".$p."_varians") == ""){
						if(empty($MessageProduct)){
							$isCartValid = false;
							$MessageProduct = array("product"=>$FormCartProduct,"message"=>"Tidak ada data varian yang dikirim [19]");
							array_push($MessageProducts,$MessageProduct);
						}
					}
						...
						if($this->response->post("cart_product".$p."_varian".$v) != "" && $this->response->postDecode("cart_product".$p."_varian".$v) != ""){
							...
							if(empty($QCartVarian)){
								...
								if(empty($QVarian)){
									if(empty($MessageProduct)){
										$isCartValid = false;
										$MessageProduct = array("product"=>$FormCartProduct,"message"=>"Varian sudah tidak tersedia [21]");
										array_push($MessageProducts,$MessageProduct);
									}
								}
									...
									if($this->response->postDecode("cart_product".$p."_varian".$v."_quantity") != "0"){
										if($QProduct->stock_type == 1 && $QProduct->stock_type_detail == 0){
											if($QVarian->stock_qty < intval($this->response->postDecode("cart_product".$p."_varian".$v."_quantity"))){
												if(empty($MessageProduct)){
													$isCartValid = false;
													$MessageProduct = array("product"=>$FormCartProduct,"message"=>"Stok tidak mencukupi [22]");
													array_push($MessageProducts,$MessageProduct);
												}
											}
										}
					...
					if($QProduct->min_order > $buy_qty){
						if(empty($MessageProduct)){
							$isCartValid = false;
							$MessageProduct = array("product"=>$FormCartProduct,"message"=>"Jumlah pembelian \"".$QProduct->name."\" kurang dari minimal order [23]");
							array_push($MessageProducts,$MessageProduct);
						}
					}
				}
			...
			if($isCartValid){
				...
				$this->response->send(array("result"=>0,"message"=>"Cart tidak valid [25]","message_products"=>$MessageProducts,"messageCode"=>25), true);
			}
		} catch (Exception $e) {
			$this->response->send(array("result"=>0,"message"=>"Server Error : ".$e,"messageCode"=>9999), true);
		}
