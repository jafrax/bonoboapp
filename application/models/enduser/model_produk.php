...
{	
	...
	function get_produk_by_id($id,$active=1){
		$this->db->select('p.id id,p.name name,p.stock_type stock_type,p.stock_type_detail stock_type_detail,p.active active,c.name kategori,p.sku_no sku_no')->where('c.toko_id',$id)->where('p.stock_type','1');
